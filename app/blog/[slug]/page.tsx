...
function AyahBlock({ num, arabic, translation }: { num: number; arabic: string; translation?: string }) {
  return (
    <div className="border border-gray-100 rounded-xl bg-gray-50 p-5 mb-3">
      <div className="flex items-start justify-between gap-3 mb-3">
        <span className="w-7 h-7 rounded-full bg-navy-900 text-white text-xs font-semibold flex items-center justify-center flex-shrink-0">{num}</span>
...
function ArabicCard({ arabic, transliteration, meaning, source }: { arabic: string; transliteration: string; meaning: string; source?: string }) {
  return (
    <div className="bg-navy-900 rounded-2xl p-6 my-6">
      <p className="text-right text-2xl sm:text-3xl leading-loose font-arabic text-white mb-4" dir="rtl" lang="ar">{arabic}</p>
      <p className="text-orange-500 text-sm font-semibold mb-1 italic">{transliteration}</p>
      <p className="text-navy-100 text-sm leading-relaxed">{meaning}</p>
      {source && <p className="text-navy-100/60 text-xs mt-2">— {source}</p>}
    </div>
  );
}
...
function SectionDivider({ label }: { label: string }) {
  return (
    <div className="flex items-center gap-4 my-8">
      ...
      <span className="text-xs font-semibold uppercase tracking-wider text-orange-500 px-3 py-1 rounded-full border border-orange-500/30">{label}</span>
      <div className="flex-1 h-px bg-orange-500/30" />
    </div>
  );
}
...
function SafetyChecklistContent() {
  return (
    <div className="text-gray-700 leading-relaxed space-y-4 [&_h2]:text-2xl [&_h2]:font-semibold [&_h2]:text-navy-900 [&_h2]:mt-10 [&_h2]:mb-3 [&_h3]:text-xl [&_h3]:font-semibold [&_h3]:text-navy-900 [&_h3]:mt-7 [&_h3]:mb-2 [&_ul]:list-disc [&_ul]:pl-5 [&_ul]:space-y-2 [&_li]:text-gray-700 [&_p]:text-[15px] [&_p]:leading-[1.8]">
      ...
      <p>For many UK Muslim families, this isn&apos;t a preference. It&apos;s a requirement, especially for daughters as they get older. Ask specifically: is there a named <Link href="/female-quran-teachers" className="text-orange-500 hover:underline font-medium">female Quran teacher</Link> available, with her own credentials, or is &ldquo;female teachers available&rdquo; just a line on the website with nobody actually behind it? You should get a name and a real answer.</p>
      ...
      <ul>
        <li>Every teacher is certified and interviewed. Our founder, <Link href="/about" className="text-orange-500 hover:underline font-medium">Shah Zaib</Link>, holds Wifaq ul Madaris Al-Arabia certification and has taught English-speaking students for over six years.</li>
        ...
        <li>We have a named, qualified female teacher, <Link href="/female-quran-teachers" className="text-orange-500 hover:underline font-medium">Almas Fatima</Link>, a Qaria e Quran with a degree in Islamic Studies, available for sisters and daughters of any age.</li>
        <li>Lessons run on Zoom or Google Meet, platforms you already know and can verify yourself.</li>
      </ul>
      <p>You can read our full policy on our <Link href="/safety" className="text-orange-500 hover:underline font-medium">Child Safety page</Link>.</p>
      <h2>So, is it safe?</h2>
      ...
      <p><Link href="/free-trial" className="text-orange-600 font-semibold hover:underline">Book your free trial class</Link>{" "}&middot;{" "}<Link href="/blog/quran-verses-to-memorize-first" className="text-orange-500 font-semibold hover:underline">What Quran Verses to Teach Your Child First</Link></p>
    </div>
  );
}
...
async function SurahAlKahfContent() {
  ...
  return (
    <div className="text-gray-700 space-y-4 [&_h2]:text-2xl [&_h2]:font-semibold [&_h2]:text-navy-900 [&_h2]:mt-10 [&_h2]:mb-3 [&_h3]:text-xl [&_h3]:font-semibold [&_h3]:text-navy-900 [&_h3]:mt-7 [&_h3]:mb-2 [&_p]:text-[15px] [&_p]:leading-[1.8]">
      ...
      {/* Jump links */}
      <div className="bg-gray-50 rounded-2xl border border-gray-100 p-5 my-6">
        <p className="text-xs font-semibold uppercase tracking-wider text-navy-900 mb-3">Jump to Section</p>
      ...
      <p>A child who can recite Surah Al-Kahf with correct tajweed and understands its meaning will carry this habit for life. This is exactly where proper Quran education makes the biggest difference. <Link href="/blog/quran-verses-to-memorize-first" className="text-orange-500 hover:underline font-medium">Read our guide on which Quran verses to memorize first.</Link></p>

      <div className="bg-navy-900 rounded-2xl p-6 my-6 text-white">
        <p className="text-orange-500 text-xs font-semibold uppercase tracking-wider mb-2">Free Trial Class</p>
        <h3 className="font-semibold text-lg mb-2">Give your child the gift of reading the Quran with confidence</h3>
        <p className="text-navy-100 text-sm mb-4 leading-relaxed">Learning to recite Surah Al-Kahf fluently and with proper tajweed takes consistent, one-on-one guidance. Our qualified teachers work with children and adults alike to build real, lasting Quran recitation skills.</p>
        <Link href="/free-trial" className="btn-gold text-sm px-5 py-2.5">Book a free trial class <ChevronRight size={14} /></Link>
      </div>
    </div>
  );
}
...
async function QuranVersesContent() {
  ...
  return (
    <div className="text-gray-700 space-y-4 [&_h2]:text-2xl [&_h2]:font-semibold [&_h2]:text-navy-900 [&_h2]:mt-10 [&_h2]:mb-3 [&_h3]:text-xl [&_h3]:font-semibold [&_h3]:text-navy-900 [&_h3]:mt-7 [&_h3]:mb-2 [&_p]:text-[15px] [&_p]:leading-[1.8]">
      ...
      <div className="bg-gray-50 rounded-2xl border border-gray-100 p-5 my-4">
        <p className="text-xs font-semibold uppercase tracking-wider text-navy-900 mb-3">Surah Al-Fatiha (1:1–7)</p>
        {fatiha.map((a) => (
      ...
      <div className="bg-gray-50 rounded-2xl border border-gray-100 p-5 my-4">
        <p className="text-xs font-semibold uppercase tracking-wider text-navy-900 mb-3">Al-Baqarah 2:285–286</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 my-4">
        {[
          ...
          <div key={s.name} className="bg-gray-50 rounded-2xl border border-gray-100 p-4">
            <p className="text-xs font-semibold uppercase tracking-wider text-navy-900 mb-3">{s.name}</p>
            {s.ayahs.map((a) => (
      ...
      <div className="bg-navy-900 rounded-2xl p-6 my-4">
        <p className="text-xs font-semibold uppercase tracking-wider text-orange-500 mb-3">Ayatul Kursi, Al-Baqarah 2:255</p>
      ...
      <p>At Ease Quran UK, our teachers guide students step by step, from their very first verses through to full Hifz, with proper tajweed built in from day one. You can also <Link href="/blog/dua-for-success" className="text-orange-500 hover:underline font-medium">read our guide on duas for success</Link> to pair your memorization journey with sincere supplication.</p>

      <div className="bg-navy-900 rounded-2xl p-6 my-6 text-white">
        <p className="text-orange-500 text-xs font-semibold uppercase tracking-wider mb-2">Start Today</p>
        <h3 className="font-semibold text-lg mb-2">Give your child the right foundation from day one</h3>
        <p className="text-navy-100 text-sm mb-4 leading-relaxed">Our qualified teachers build proper tajweed and memorization habits from the very first lesson, not corrected after the fact.</p>
        <Link href="/free-trial" className="btn-gold text-sm px-5 py-2.5">Book a free trial class <ChevronRight size={14} /></Link>
      </div>
    </div>
  );
...
async function DuaForSuccessContent() {
  ...
  return (
    <div className="text-gray-700 space-y-4 [&_h2]:text-2xl [&_h2]:font-semibold [&_h2]:text-navy-900 [&_h2]:mt-10 [&_h2]:mb-3 [&_h3]:text-xl [&_h3]:font-semibold [&_h3]:text-navy-900 [&_h3]:mt-7 [&_h3]:mb-2 [&_p]:text-[15px] [&_p]:leading-[1.8]">
      ...
      <p>At Ease Quran UK, our teachers help students build a real connection to the Quran and daily duas. Building this weekly habit of reading Surah Al-Kahf is another great complement to your dua practice, <Link href="/blog/surah-al-kahf-friday-reading-guide" className="text-orange-500 hover:underline font-medium">read our full Surah Al-Kahf guide.</Link></p>

      <div className="bg-navy-900 rounded-2xl p-6 my-6 text-white">
        <p className="text-orange-500 text-xs font-semibold uppercase tracking-wider mb-2">Start Your Journey</p>
        <h3 className="font-semibold text-lg mb-2">Build a confident, meaningful relationship with the Quran</h3>
        <p className="text-navy-100 text-sm mb-4 leading-relaxed">Our teachers combine proper tajweed with meaning-based learning so recitation becomes second nature, not just memorized sound.</p>
        <Link href="/free-trial" className="btn-gold text-sm px-5 py-2.5">Book a free trial class <ChevronRight size={14} /></Link>
      </div>
    </div>
  );
...
export default async function BlogPost({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      ...
      {/* Hero */}
      <section className="relative bg-navy-900 overflow-hidden min-h-[320px] sm:min-h-[380px] flex items-end">
        ...
        <div className="relative z-10 w-full max-w-container mx-auto px-4 sm:px-6 pb-10">
          <Breadcrumb crumbs={[{ label: "Home", href: "/" }, { label: "Blog", href: "/blog" }, { label: post.category }]} dark />
          <div className="mt-4 max-w-3xl">
            <div className="flex flex-wrap items-center gap-3 mb-3">
              <span className="text-xs font-semibold text-navy-900 bg-orange-500 px-3 py-1 rounded-full">{post.category}</span>
              <span className="text-navy-100 text-xs flex items-center gap-1"><Clock size={11} /> {post.readTime}</span>
              <time className="text-navy-100 text-xs" dateTime={post.date}>
                {new Date(post.date).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}
              </time>
            </div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-semibold text-white leading-snug">{post.title}</h1>
          </div>
        </div>
      </section>
            ...
            <article>
                  ...
                  <div className="space-y-3">
                    {post.faqs.map((f, i) => (
                      <details key={i} className="bg-gray-50 rounded-xl border border-gray-100 group">
                        <summary className="flex justify-between items-center p-5 cursor-pointer font-semibold text-navy-900 list-none text-sm">
                          {f.q}
              ...
              <div className="mt-12 border-t border-gray-100 pt-8">
                <div className="flex items-start gap-5 bg-gray-50 rounded-2xl p-6">
                  <div className="relative w-16 h-16 rounded-full flex-shrink-0 overflow-hidden border-2 border-orange-500">
                    <Image src="/founder.webp" alt="Shah Zaib, founder of Ease Quran UK" fill className="object-cover object-top" sizes="64px" />
                  </div>
                  <div>
                    <p className="font-semibold text-navy-900">Shah Zaib</p>
                    <p className="text-xs text-orange-500 font-semibold mb-2">Founder, Ease Quran UK &bull; Wifaq ul Madaris Al-Arabia Certified</p>
                    ...
                    <Link href="/about" className="inline-flex items-center gap-1 text-orange-500 text-xs font-semibold mt-2 hover:underline">
              ...
              <div className="mt-10 border-t border-gray-100 pt-8">
                <p className="text-sm font-semibold text-navy-900 mb-4">Explore Related Pages</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {relatedPages.map((r) => (
                    <Link key={r.href} href={r.href} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-50 border border-gray-100 text-sm font-semibold text-navy-900 hover:border-orange-500/50 hover:shadow-sm transition-all group">
                      <span className="text-orange-500">{r.icon}</span>
                      {r.label}
                      <ChevronRight size={12} className="ml-auto text-gray-300 group-hover:text-orange-600 transition-colors" />
                    </Link>
                  ))}
                </div>
              </div>
            ...
            <aside className="hidden lg:block">
              <div className="sticky top-24 space-y-5">
                <div className="bg-navy-900 rounded-2xl p-6 text-white">
                  <p className="text-orange-500 text-xs font-semibold uppercase tracking-wider mb-2">Free Trial Class</p>
                  <h3 className="font-semibold text-lg mb-2 leading-snug">Try a free Quran lesson</h3>
                  <p className="text-navy-100 text-xs mb-4 leading-relaxed">No card needed. No commitment. One-to-one with a certified teacher, confirmed within 2 hours.</p>
                  <Link href="/free-trial" className="btn-gold w-full justify-center text-sm py-2.5">Book free trial</Link>
                </div>

                <a href={`https://wa.me/447311254423?text=${waMsg}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 bg-whatsapp text-white rounded-2xl p-4 hover:opacity-90 transition-colors">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-white/80">WhatsApp us directly</p>
                  </div>
                </a>

                <div className="bg-white rounded-2xl border border-gray-100 p-5">
                  <p className="text-xs font-semibold uppercase tracking-wider text-navy-900 mb-4">More from the Blog</p>
                  <ul className="space-y-3">
                    {allPosts.filter(p => p.slug !== slug).map((p) => (
                      <li key={p.slug}>
                        <Link href={`/blog/${p.slug}`} className="flex items-start gap-2 text-sm text-gray-600 hover:text-orange-600 transition-colors leading-snug">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-5">
                  <p className="text-xs font-semibold uppercase tracking-wider text-navy-900 mb-4">Quick Links</p>
                  <ul className="space-y-2">
                    {[
                      ...
                      <li key={l.href}>
                        <Link href={l.href} className="flex items-center gap-2 text-sm text-gray-600 hover:text-orange-600 transition-colors">
